...
public class warmanemily {
	...
	public static boolean voteReport(ArrayList<Candidate> alc) {
		...
		if (spaceCounter == 1 || userInput.equals("all")) {
			if (userInput.equals("all")) {
				...
				ArrayList<ArrayList<Candidate>> offices = new ArrayList<ArrayList<Candidate>>();
				offices.add(presidents);
				offices.add(governors);
				offices.add(mayors);
				offices.add(instructors);
				for (int i = 0; i<alc.size(); i++) {
				...
				for(int i=0; i<states.size(); i++) {
					thisState = states.get(i);
					//get elections, calculate total votes
					for(int k=0;k<offices.size();k++) {
						ArrayList<Candidate> electionRace = offices.get(k);
						System.out.println(thisState.toUpperCase() +" "+electionRace.get(0).getOffice().toUpperCase()+"S");
						for(int j=0; j<electionRace.size(); j++) {
							if(electionRace.get(j).getState().equalsIgnoreCase(thisState)){
								election.add(electionRace.get(j));
								totalVotes+=electionRace.get(j).getVotes();
							}
						}
						if (election.size()==0) {
							System.out.println("none");
						}
						//print elections, calculate percent votes
						for (int j=0; j<election.size(); j++) {
							Candidate oC = election.get(j);
							double percentVotes = (oC.getVotes()/totalVotes)*100;
	...
	public static boolean partyReport(ArrayList<Candidate> alc) {
		...
		ArrayList <Candidate> other = new ArrayList<Candidate>();
		Scanner scan = new Scanner(System.in);
		System.out.println("Enter party you'd like (full name or char) to search OR 'all': ");
		String searchName = scan.nextLine().toLowerCase();
		//sorting by parties
		for (int i = 0; i<alc.size(); i++) {
			char party = alc.get(i).getParty();
	...
	/*The sort list method displays all the candidates in alphabetical order*/
	public static boolean sortList(ArrayList<Candidate> alc) {
		boolean flag = true;
		Candidate temp;
		while(flag) {
			flag = false;
			for(int i=0; i<alc.size()-1; i++) {
				if (alc.get(i).getName().compareTo(alc.get(i+1).getName())>0) {
					temp = alc.get(i);
		...
		for(int i=0; i<alc.size(); i++) {
			System.out.println(alc.get(i).getName());
		}
		return true;
	}
	...
	public static boolean finalStats(ArrayList<Integer>counter)throws IOException {
		...
		System.out.printf("%-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %n", 'L', 'C', 'V', 'S', 'D', 'P', 'O', 'Q', "Bad");
		System.out.printf("%-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %n", counter.get(0), counter.get(1), counter.get(2),counter.get(3), counter.get(4), counter.get(5), counter.get(6), counter.get(7), counter.get(8));
		pw.printf("%-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %n", 'L', 'C', 'V', 'S', 'D', 'P', 'O', 'Q', "Bad");
		pw.printf("%-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s", counter.get(0), counter.get(1), counter.get(2),counter.get(3), counter.get(4), counter.get(5), counter.get(6), counter.get(7), counter.get(8));
